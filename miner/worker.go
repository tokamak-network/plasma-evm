...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, pls Backend, env *epoch.EpochEnvironment, mux *event.TypeMux, db ethdb.Database, isLocalBlock func(*types.Block) bool, init bool) *worker {
	worker := &worker{
		config:             config,
	...
	worker.txsSub = pls.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = pls.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = pls.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
...
func (w *worker) commitRequestTransactions(rtxs types.Transactions, coinbase common.Address, interrupt *int32) bool {
	...
	for _, rtx := range rtxs {
		...
		// phase, start ignoring the sender until we do.
		if rtx.Protected() && !w.chainConfig.IsEIP155(w.current.header.Number) {
			log.Trace("Ignoring reply protected transaction", "hash", rtx.Hash(), "eip155", w.chainConfig.EIP155Block)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	pending, err := w.pls.TxPool().Pending()
	if err != nil {
	...
	for _, account := range w.pls.TxPool().Locals() {
