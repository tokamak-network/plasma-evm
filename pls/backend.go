...
func (s *Plasma) SetContractBackend(backend bind.ContractBackend) {
...
func New(ctx *node.ServiceContext, config *Config) (*Plasma, error) {
	...
	chainConfig, genesisHash, genesisErr := core.SetupGenesisBlockWithOverride(chainDb, config.Genesis, config.RootChainContract, config.Operator.Address, config.StaminaConfig, ctx.ResolvePath(""), config.OverrideIstanbul, config.OverrideMuirGlacier)
	...
	log.Info("Initialising Plasma protocol", "versions", ProtocolVersions, "network", config.NetworkId, "dbversion", dbVer)
	...
	pls.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, chainConfig, pls.engine, vmConfig, pls.shouldPreserve)
	if err != nil {
		return nil, err
	}
	...
	if pls.protocolManager, err = NewProtocolManager(chainConfig, checkpoint, config.SyncMode, config.NetworkId, pls.eventMux, pls.txPool, pls.engine, pls.blockchain, chainDb, cacheLimit, config.Whitelist); err != nil {
		return nil, err
	}

	epochEnv := rawdb.ReadEpochEnv(chainDb)

	pls.miner = miner.New(pls, &config.Miner, chainConfig, pls.EventMux(), pls.engine, epochEnv, chainDb, pls.isLocalBlock)
	pls.miner.SetExtra(makeExtraData(config.Miner.ExtraData))

	pls.APIBackend = &PlsAPIBackend{ctx.ExtRPCEnabled(), pls, nil}
...
func (s *Plasma) isLocalBlock(block *types.Block) bool {
...
// is a local account.
func (s *Plasma) shouldPreserve(block *types.Block) bool {
...
func (s *Plasma) ArchiveMode() bool                  { return s.config.NoPruning }
