...
func New(url string, plsServ *pls.Plasma, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.pls != nil {
		...
		if block == nil {
			block = s.pls.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.pls.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.pls != nil {
			head = s.pls.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.pls != nil {
			block = s.pls.BlockChain().GetBlockByNumber(number)
		} else {
